...
// Utility functions
const getFontAwesomeIconName = (iconName) => {
  return iconName
    .replace(/^fa/, '')
    .replace(/([A-Z])/g, '-$1')
    .toLowerCase()
...
const createIconListFromPack = (iconPack, packName) => {
  return Object.keys(iconPack).map(iconName => ({
    ...
    pack: packName,
    iconName: getFontAwesomeIconName(iconName),
    displayName: getIconDisplayName(iconName)
  }));
};
...
const iconListWithPack = allIconPacks
  .flatMap(({ icons, name }) => createIconListFromPack(icons, name));
              ...
              <b-card-body class="d-flex justify-content-between align-items-center">
                <ul class="icon-list space-top-icon">
                  <li
                      ...
                      :key="icon.id"
                      class="icon-box"
                      @click="copyIcon(icon.iconName, icon.pack)"
                  >
                    <FontAwesomeIcon
                        class="fontawesome-icons"
                        size="2xl"
                        :icon="[icon.pack, icon.iconName]"
                    />
